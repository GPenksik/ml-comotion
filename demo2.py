...
# INPUT_PATH = "samples/IronShot3_FaceOn.mp4"
INPUT_PATH = "samples/IronShot3_DTL.mp4"
OUTPUT_PATH = "results/IronShot3"
...
def track_poses(
    ...
    # Stack detection data similar to tracks
    stacked_detections = {}
    for k in ["pred_3d", "pred_2d", "conf"]:
        if k in detections:
            stacked_detections[k] = torch.stack(detections[k], 0)
    ...
    if track_ref:
        ...
        # Add detection data to saved predictions
        for k in ["pred_3d", "pred_2d", "conf"]:
            if k in stacked_detections:
                # Extract detection data for the corresponding frames and tracks
                preds[k] = stacked_detections[k][frame_idxs, track_idxs]
